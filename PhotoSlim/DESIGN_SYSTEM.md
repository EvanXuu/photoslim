...
- 文案：直接说明动作及其对象。主按钮使用“检查空间并开始”“加入准备队列”“确认删除原件”等单一动词意图；错误说明发生了什么、原件是否安全、下一步是什么。
...
- 浏览 Toolbar：搜索、筛选、排序、列表/网格切换、全选/取消全选和扫描变更集中在窗口顶部，使用 Finder 风格的原生 Toolbar 控件；列表和网格都支持通过上下文菜单置顶，置顶项目稳定排在当前排序之前。
- 页面结构：侧栏 -> 页面标题/工具 -> 主数据视图 -> 选择条。处理页和审核页临时替代主数据视图。
- 选择条：选中后同时显示精确/估算输入、预计节省、任务临时空间和本机可用于任务的空间；容量校验结果不得延迟到开始按钮之后。
- iCloud 项目在下载前不估算原件、输出或节省空间；开始任务后显示独立的下载进度，下载完成才读取真实文件大小，并用真实输出大小复核压缩率。
...
- 表单：系统 Form、Picker、Slider、Stepper、TextField、Toggle、DatePicker；视频参数按“码率策略 -> 码率 -> 速率上限 -> 关键帧/帧重排 -> 音频”顺序排列，帮助文字紧邻对应设置。
- 进度：全局进度一条；当前项固定展示“iCloud 下载”和“HEIC/HEVC 压缩”两条独立进度。
- 视频编码：普通 SDR H.264 和普通 hvc1 HEVC 使用 AVAssetReader/AVAssetWriter 配合 VideoToolbox 手动控制 HEVC 码率；HDR、定时元数据或手动管线不适配时使用 AVAssetExportSession 的兼容 HEVC 预设。共享的 HEVC 硬件能力模块最低支持 iOS 15，桌面 UI 仍支持 macOS 14+。
...
- 结果预览：结果网格使用本地临时输出；点击进入单项预览，按住按钮或键盘反斜杠临时显示对应原图，松开立即恢复压缩结果。
