...
- 产品：PhotoSlim。
- 类型：原生 macOS/iOS 照片生产力工具；App/Product UI + Native Desktop/Mobile。
- 平台：macOS 14 及以上、iOS 17 及以上，SwiftUI。
- 用户：拥有大型 Apple/iCloud 照片图库、重视隐私与可逆操作的 Apple 用户。
...
- 文案：直接说明动作及其对象。主按钮使用“开始压缩”“加入准备队列”“确认删除原件”等单一动词意图；错误说明发生了什么、原件是否安全、下一步是什么。面向用户的文案不暴露 PhotoKit、资源读取、sample entry、内部校验或编码器实现；格式、空间、风险和下一步仍要说清楚。
...
- 参考：
  ...
  - [Apple tab bars](https://developer.apple.com/design/human-interface-guidelines/tab-bars)：iPhone 顶层导航保持稳定、可发现。
  - [Apple lists and tables](https://developer.apple.com/design/human-interface-guidelines/lists-and-tables)：移动端使用分组列表承载高密度图库项目。
  - [Apple navigation and search](https://developer.apple.com/design/human-interface-guidelines/navigation-and-search)：搜索放在导航上下文或内容区，不覆盖主要内容。
...
- 浏览 Toolbar：搜索、筛选、排序、列表/网格切换、全选/取消全选和扫描变更集中在窗口顶部，使用 Finder 风格的原生 Toolbar 控件；排序按钮展开后直接列出排序方式，并用勾选标记当前方式，不再增加同名二级菜单；使用原生窗口标题显示当前媒体类型，使用原生窗口副标题显示项目数量，避免将标题放入 Toolbar 气泡；列表和网格都支持通过上下文菜单置顶，置顶项目稳定排在当前排序之前。
- 页面结构：侧栏 -> 顶部 Toolbar -> 主数据视图 -> 选择条。处理页和审核页临时替代主数据视图。
- 选择条：无论是否选中项目，都以“可用空间 · 总容量”显示当前设备存储状态；选中后只增加项目数量，不生成输出大小、所需空间或节省空间预测。点击开始后在后台完成容量与安全余量校验，只有无法继续时才显示可操作的错误。
...
- 主流程：浏览/筛选 -> 选择 -> 后台容量安全校验 -> 最多 5 路 iCloud 下载/处理 -> 本地结果预览 -> 用户确认写入 -> 强制审核/系统删除 -> 两种终局之一。
- 模态：详细设置使用 sheet；容量校验不打断流程，也不展示独立报告；排除项与撤回风险由应用说明；最终删除直接使用 PhotoKit 触发的 Photos 系统确认，不叠加应用内 alert；处理可以收为右下角非模态浮窗。
...
- 视频编码：普通 SDR H.264 和普通 hvc1 HEVC 默认走 AVAssetExportSession 的 HEVC 预设；用户在视频详细设置中显式切换手动后，才使用 AVAssetReader/AVAssetWriter 配合 VideoToolbox 控制 HEVC 码率。手动码率表按 1080p/2160p/4320p 与 30/60 fps 编辑，非标准尺寸按像素数插值；HDR、定时元数据或手动管线不适配时仍使用兼容预设。共享的 HEVC 硬件能力模块最低支持 iOS 17，桌面 UI 仍支持 macOS 14+。
- 数据状态：每个数据页都有空、加载、失败状态；失败文案同时给出原件安全状态。
- 图表：统计页使用原生几何条形，不引入图表库；顶部先显示本机存储容量，再显示累计节省。
...
- iOS 与 macOS 直接共享 `AppModel`、资产/筛选/设置/会话模型，以及 PhotoKit、磁盘检查、压缩和持久化服务；iPhone 只提供符合触控习惯的导航壳，不维护第二套任务逻辑。
- 图库使用 `NavigationStack`：左上角媒体类型菜单切换照片图库、照片、视频和收藏；右上角只保留筛选与更多。排序、列表/网格、全选、压缩参数、扫描变更和刷新存储统一收进“更多”。
- 搜索是图库内容顶部的系统语义输入框。初次进入时滚动位置位于搜索栏下方；用户向下拉动图库后搜索栏出现并停留，避免启动时挤占一行媒体空间，也避免 iOS 26 `searchable` 浮层遮挡内容。
- 未选择项目时，底部保留图库、队列、统计、历史四个稳定工作区；最小化任务可使用系统底部附件显示进度。
- 选中项目后隐藏工作区栏，并在同一安全区域显示一条独立胶囊状态栏：左侧两行显示“已选 N 项”和“可用空间 · 总容量”，右侧依次为灰色半透明胶囊“参数”和 Signal 色胶囊“下一步”。iOS 26 及以上使用原生 Liquid Glass 承载整条状态栏；iOS 17–25 使用系统 Material 与细描边回退，按钮结构和 44 pt 最小触控高度不变。取消全部选择后恢复四个工作区。
- “下一步”在后台执行空间与安全余量校验，成功后直接进入 iCloud 原件准备和压缩；空间不足时只显示下一步操作，当前任务占用执行器时则加入队列。
- 参数 Sheet 根据实际上下文裁剪：纯照片选择只显示照片质量，纯视频选择只显示视频编码；视频“自动”只显示自动/手动选择，手动后才展开 1080p/2160p/4320p × 30/60 fps 码率表、帧重排和音频策略。任务安全默认 8%。
- 浏览区使用 LazyVStack 列表或双列 LazyVGrid。列表行最小高度 80 pt，图标和按钮触控区域不小于 44 pt；网格缩略图保持 4:3 容器并使用 scaledToFit，不裁切照片。
- iOS 缩略图只读取 PhotoKit 已可用的资源，`isNetworkAccessAllowed = false`，不因浏览界面触发 iCloud 原件下载；云端状态只显示云图标，资源大小未知时不渲染大小文本。
- 处理页分别显示 iCloud 下载和压缩进度，并提供最小化与终止；结果页先审核本地临时输出，支持点按放大和按住查看原图，确认后才写入相册并请求 Photos 删除原件。
- iOS 首页不显示索引时间、硬件编码能力和接口名称等开发者信息；错误只说明用户需要采取的下一步，并明确原件是否安全。内容层使用系统 grouped background，只有系统导航和底部操作层使用平台材质。
